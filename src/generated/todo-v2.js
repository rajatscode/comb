...
export function TodoApp(root) {
  ...
  const isEmpty = createComb(() => (count() == 0), { name: 'isEmpty', module: $m, deps: ["count"] });

  const remaining = createComb(() => count(), { name: 'remaining', module: $m, deps: ["count"] });

  const label = createComb(() => (String(remaining()) + " remaining"), { name: 'label', module: $m, deps: ["remaining"] });

  const visible = createComb(() => ((filter() == "all") ? items() : ((filter() == "done") ? items() : items())), { name: 'visible', module: $m, deps: ["filter","items"] });

  const showClear = createComb(() => !isEmpty(), { name: 'showClear', module: $m, deps: ["isEmpty"] });
...
export function __test() {
  ...
  return {
    ...
    combs: { count, isEmpty, remaining, label, visible, showClear },

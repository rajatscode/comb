...
// Tracks toggle coverage (boolean signals), FSM transition coverage, and cross coverage.
...
export class CoverageCollector {
   ...
   * The total number of possible combinations is 2^n for n boolean signals.
   */
   ...
   * Values array must match the order of signalIds from registerCrossGroup.
   */
   ...
   *
   * Coverage points:
   * - Toggle: 2 points per signal (seen true + seen false)
   * - Cross: one point per possible combination (2^n for n signals)
   * - Transitions are reported but not counted in the percentage
   *   (total possible transitions depends on the FSM definition)
   */
  getReport(): CoverageReport {
    ...
    // Cross: each possible combo is a point
